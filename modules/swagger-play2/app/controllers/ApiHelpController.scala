      ...
      Option(responseStr) match {
        ...
        case None => {
          ...
          returnXml(request) match {
            case true => {
              Result(header = ResponseHeader(500), body = play.api.libs.iteratee.Enumerator(toXmlString(msg).getBytes())).as("application/xml")
            }
            case false => {
              Result(header = ResponseHeader(500), body = play.api.libs.iteratee.Enumerator(toJsonString(msg).getBytes())).as("application/json")
            }
          }
        }
      }
...
class SwaggerBaseApiController extends Controller {
  ...
  protected def XmlResponse(data: Any) = {
    ...
    Result(header = ResponseHeader(200), body = play.api.libs.iteratee.Enumerator(xmlValue.getBytes())).as("application/xml")
  }
  ...
  protected def JsonResponse(data: Any) = {
    ...
    Result(header = ResponseHeader(200), body = play.api.libs.iteratee.Enumerator(jsonValue.getBytes())).as("application/json")
  }
}
